...
export default function Player() {
  ...
  return (
    <div className="bg-zinc-900/95 backdrop-blur-xl border-t border-white/5">
      {/* Progress bar — always visible, thin strip at top */}
      <div className="relative h-1 w-full bg-zinc-800">
        <div className="absolute inset-y-0 left-0 bg-violet-500 transition-all"
      ...
      <div className="flex items-center px-4 gap-3 h-16">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-zinc-400 truncate">{currentSong.singer || currentSong.artist}</p>
          </div>
        </div>

        {/* Mobile: just Prev + Play + Next */}
        <div className="flex items-center gap-3 md:hidden">
          ...
          <button onClick={togglePlay}
            className="w-9 h-9 rounded-full bg-white flex items-center justify-center hover:scale-105 active:scale-95 transition-transform shadow-lg">
            {isPlaying
              ? <svg viewBox="0 0 24 24" width="18" height="18" fill="#09090b"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg>
              : <svg viewBox="0 0 24 24" width="18" height="18" fill="#09090b"><path d="M8 5v14l11-7z"/></svg>
            }
          </button>
          <IconBtn onClick={handleNext} title="Next">
            <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zm2-8.14L11.03 12 8 14.14V9.86zM16 6h2v12h-2z"/></svg>
          </IconBtn>
        </div>
